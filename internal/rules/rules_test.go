...
func TestEngineShouldProxy(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "cidr /32 excludes exact public IP",
			cfg:  Config{NoProxyIPRanges: []string{"8.8.8.8/32"}},
			ip:   net.ParseIP("8.8.8.8"),
			want: false,
		},
		{
			name: "cidr /32 does not exclude adjacent public IP",
			cfg:  Config{NoProxyIPRanges: []string{"8.8.8.8/32"}},
			ip:   net.ParseIP("8.8.8.9"),
			want: true,
		},
		{
			name: "cidr /32 coexists with broader range",
			cfg:  Config{NoProxyIPRanges: []string{"10.0.0.0/8", "8.8.8.8/32"}},
			ip:   net.ParseIP("8.8.8.8"),
			want: false,
		},
		{
			name: "cidr /32 coexisting broader range still excludes",
			cfg:  Config{NoProxyIPRanges: []string{"10.0.0.0/8", "8.8.8.8/32"}},
			ip:   net.ParseIP("10.1.2.3"),
			want: false,
		},
		{
			name: "cidr /32 coexisting broader range allows outside",
			cfg:  Config{NoProxyIPRanges: []string{"10.0.0.0/8", "8.8.8.8/32"}},
